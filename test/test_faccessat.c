...
int main(int argc, char *argv[])
{
  ...
  // See manual_tests/do_test_faccessat_setuid.sh for details.
  ...
  if (argc > 1)
  {
    ...
    for (a = 1; a < argc; ++a)
    {
      for (m = 0; m < 8; ++m)
      {
        int faccessat_rc = faccessat(AT_FDCWD, argv[a], modes[m], 0);
        int faccessat_errno = errno;
        int access_rc = access(argv[a], modes[m]);
        ...
        if (faccessat_rc != access_rc || (faccessat_rc < 0 && faccessat_errno != access_errno))
        {
          ...
          char buf[1024];
          snprintf(buf, 1024, "/bin/ls -l %s", argv[a]);
          system(buf);
        }
      }
    }
  ...
  (void) snprintf(tmpdir, sizeof(tmpdir), TEST_TEMP "/%s-%u",
                  progname, pid);

  #define SETUP_PATH(name) \
    (void) snprintf(name##_path, sizeof(name##_path), "%s/" #name, tmpdir)
  SETUP_PATH(readable);
  SETUP_PATH(writable);
  SETUP_PATH(executable);
  SETUP_PATH(inaccessible);
  SETUP_PATH(nonexistent);

  /* Create temporary directory if not already there (see make test_faccessat_setuid) */

  int mkdir_rc = mkdir(tmpdir, 0700);
